...
\$(document).ready(function() {
			...
			//FULL view on load	
			window.onload =	\$('#tabFULLSPLIT_wrapper .dataTables_scrollHead thead tr:eq(1) th:eq('+fullField+') input', \$('.tooltipHeader td')[fullField] ).val('full' ).change();
				...
				\$('#tabFULLSPLIT tbody').on('dblclick', 'tr', function () {
					var rowID = this.id;			
					var keyType = '3';  //type field
					var keyAnnotID = '0';  //annotSV_ID field
					var fm = fullMode;
